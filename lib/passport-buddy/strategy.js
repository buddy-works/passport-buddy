 ...
 *   - `scope`         array of permission scopes to request.  required: 'USER_EMAIL', 'MANAGE_EMAILS'
...
function Strategy(options, verify) {
  ...
  this._userEmailsURL = options.userEmailsURL || 'https://api.buddy.works/user/emails';
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get(this._userProfileURL, accessToken, function (err, body) {
    ...
    self._oauth2.get(self._userEmailsURL, accessToken, function(err, body) {
      ...
      try {
        emailsJson = JSON.parse(body);
      } catch (ex) {
        return done(new Error('Failed to parse user emails'));
      }

      json.emails = emailsJson.emails || [];
